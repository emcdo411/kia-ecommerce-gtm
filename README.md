...
flowchart LR
%% ========= Styles =========
classDef milestone fill:#111827,stroke:#111827,color:#fff,rx:8,ry:8,font-weight:bold
...
IFM2 --> DEAL1[Identify stakeholders\n(Dealer Principal, Parts Mgr, IT)]:::dealer
DEAL1 --> DEC1{Dealer signs program?}:::decision
DEC1 -- "No" --> NURT[Nurture sequence\n(quarterly touches)]:::ifm --> IFM2
DEC1 -- "Yes" --> P0[Provision project in tracker]:::ifm
end

Start --> IFM1

%% ========= Phase 2: Onboard & Setup =========
subgraph P2["Phase 2 — Onboard & Setup (Days 31–60)"]
direction TB

  %% Lane A — Domain & Payments
  subgraph LNA["A) Domain & Payments"]
  direction TB
  DNS[Subdomain request + DNS CNAME/TXT]:::dealer --> PG[Payment gateway config\n(test $1 authorization)]:::dealer
  ...
  %% Fan-out from provisioning; fan-in to Go-Live Review
  P0 --> DNS
  P0 --> CAT1
  P0 --> GA4
  P0 --> SEO
